...
fmt: $(GOIMPORTS)
	...
	@$(GOIMPORTS) -local github.com/shuldan/config -w $$(find . -name "*.go" -not -path "./vendor/*")
...
fmt-check: $(GOIMPORTS)
	...
	@unformatted_imports=$$($(GOIMPORTS) -local github.com/shuldan/config -l . | grep -v vendor || true); \
